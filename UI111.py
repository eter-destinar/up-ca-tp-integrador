...
class Configuracion(QWidget):
    def __init__(self, ventana_principal):
        ...
        for i in range(CANT_TANQUES):
            ...
            boton_LeerSensor = QPushButton("Leer Sensor")
            boton_LeerSensor.setFixedWidth(80)
            boton_LeerSensor.setFixedHeight(20)  # Reduce la altura del botón
            boton_LeerSensor.setStyleSheet("""
                QPushButton {
                    font-size: 12px;
                    font-weight: bold;
                    border-radius: 5px;
                ...
                QPushButton:hover {
                    background-color: #206b23;
                }
                QPushButton:pressed {
                    background-color: #ff0800;
                }
            """)
            boton_LeerSensor.clicked.connect(lambda _, c=i+1 : self.LeerSensor(c))
            ...
            fila.addWidget(boton_LeerSensor)
            layout.addLayout(fila)
            
            
        botones_layout = QHBoxLayout()

        self.boton_cargar_datos = QPushButton("Leer Archivo .csv")
    ...
    def LeerSensor(self, indice):
        print(f"Lectura sensor ID {indice}")
        valor = self.leer_sensor(indice)
        if 0 <= indice - 1 < len(self.campos_tanques):
            self.campos_tanques[indice - 1].setText(str(valor))
    ...
    def leer_sensor(self,sensor_id, registro=1, puerto=PUERTO_SENSORES):
        try:
            instr = self.inicializar_instrumento(puerto, sensor_id)
            valor = instr.read_register(registro, 0)
            time.sleep(2)
            print(f"✅ Sensor ID {sensor_id} -> Registro {registro}: {valor}")
            return valor
        except IOError:
            QMessageBox.warning(self, "Error de lectura", f"No se pudo leer el sensor con ID {sensor_id}")
            #print(f"⚠️ Error leyendo sensor con ID {sensor_id}")
            return -1
